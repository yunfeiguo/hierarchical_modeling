...
system(paste(pimsa,"example_pimsa_settings.xml"),wait=TRUE,ignore.stdout=TRUE)
...
start_snp = read.table("models.out",as.is=TRUE,skip=1000)
...
result=sampleBVS2(start=start_model,cov=cov,inform=TRUE,mult.regions=TRUE,regions=region,iter=100,data=data,rare=TRUE,status.file="/dev/null")
summary.result = summaryBVS(result,data,rare=TRUE,mult.regions=TRUE,cov=cov,regions=region,inform=TRUE,burnin=10)
