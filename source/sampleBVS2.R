		     ...
		     iter=10000,conver.thres=NULL,conver.count=1000,save.iter=0,outfile=NULL,status.file=NULL,old.results=NULL)
{
	...
	model.type=0:1
	#this is a place we can improve
	#changing one  variable randomly obviously is not an ideal way of looking for next optimal model
	mutate.ind = sample(1:M,1)
	Z.new = Z.current
	#'sample' function doesn't seem to be necessary here
	...
	a = min(
		1,
		(exp(
		     as.numeric( fit.new[length(which.ind)+1])-
		     fit.current[length(which.ind)+1]
		     )
		)^(-1)
		)
	rand = runif(1)
